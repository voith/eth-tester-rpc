...
def emitter_contract_address(rpc_client, accounts, EMITTER_CODE):
    txn_hash = rpc_client(
        'eth_sendTransaction',
        params=[{"from": accounts[0], 'data': EMITTER_CODE}]
    )
    txn_receipt = rpc_client('eth_getTransactionReceipt', params=[txn_hash])
...
@pytest.fixture()
def rpc_emitter_contract_address(rpc_client, accounts, EMITTER_CODE):
    txn_hash = rpc_client(
...
@pytest.fixture()
def client_call_emitter(rpc_client, accounts, emitter_contract_address, EMITTER_ABI):
    def _client_call_emitter(method_signature, arguments=None):
        if arguments is None:
            arguments = []
        data = encode_fn_abi(EMITTER_ABI, method_signature, arguments)
...
def event_topic(event_signature):
    return encode_hex(keccak(text=event_signature))
